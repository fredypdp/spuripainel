...
export default function AdministradoresPageContent() {
  ...
  return <div><PageBreadcrumb pageTitle="Administradores" /><div className="space-y-6">
    ...
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]"><div className="w-full overflow-x-auto"><table className="w-full text-left text-sm"><thead className="border-b border-gray-100 dark:border-white/[0.05]"><tr>{["Nome","Email","Role","Status","Telefone","Ações"].map(h=><th key={h} className="px-5 py-3 font-medium text-gray-500 dark:text-gray-400">{h}</th>)}</tr></thead><tbody>{admins.map((admin)=><tr key={admin.id} className="border-b border-gray-100 dark:border-white/[0.05]"><td className="px-5 py-4 text-gray-800 dark:text-white/90">{admin.nome}</td><td className="px-5 py-4 text-gray-600 dark:text-gray-300">{admin.email}</td><td className="px-5 py-4 text-gray-600 dark:text-gray-300">{roleLabels[admin.role]}</td><td className="px-5 py-4 text-gray-600 dark:text-gray-300">{admin.status}</td><td className="px-5 py-4 text-gray-600 dark:text-gray-300">{admin.telefone || "—"}</td><td className="px-5 py-4"><div className="flex flex-wrap gap-2">{admin.status === "inativo" ? <Button size="sm" variant="outline" disabled={!canChangeStatus(admin) || ativar.loading} onClick={()=>handleAtivar(admin)}>Ativar</Button> : <Button size="sm" variant="outline" disabled={!canChangeStatus(admin) || desativar.loading} onClick={()=>abrirMotivo(admin,"desativar")}>Desativar</Button>}<Button size="sm" variant="danger" disabled={!canDelete(admin) || deletar.loading} onClick={()=>abrirMotivo(admin,"deletar")}>Deletar</Button>{!canDelete(admin) && <span className="text-xs text-gray-400">Sem permissão hierárquica</span>}</div></td></tr>)}{!loading && admins.length===0 && <tr><td className="px-5 py-8 text-center text-gray-500 dark:text-gray-400" colSpan={6}>Nenhum administrador encontrado.</td></tr>}</tbody></table></div></div>
  </div>
  <Modal isOpen={criarModal.isOpen} onClose={criarModal.closeModal} className="max-w-[560px] p-6 lg:p-10"><form onSubmit={handleCriar} className="space-y-4"><h4 className="text-lg font-medium text-gray-800 dark:text-white/90">Criar administrador</h4><Label>Nome *</Label><input className="w-full rounded-lg border px-4 py-3 text-sm text-gray-800 dark:bg-white/[0.03] dark:text-white/90" value={formCriar.nome} onChange={e=>setFormCriar({...formCriar,nome:e.target.value})} required/><Label>Email *</Label><input type="email" className="w-full rounded-lg border px-4 py-3 text-sm text-gray-800 dark:bg-white/[0.03] dark:text-white/90" value={formCriar.email} onChange={e=>setFormCriar({...formCriar,email:e.target.value})} required/><Label>Role *</Label><SearchableSelect value={formCriar.role} options={[{ value: "gerente", label: "Gerente" }, { value: "adm", label: "Administrador" }, ...(currentRole === "fpp" ? [{ value: "fpp", label: "FPP" }] : [])]} onChange={v=>setFormCriar({...formCriar,role:(v || "gerente") as AdminRole})} isClearable={false} /><Label>Senha inicial</Label><input type="password" className="w-full rounded-lg border px-4 py-3 text-sm text-gray-800 dark:bg-white/[0.03] dark:text-white/90" value={formCriar.senha} onChange={e=>setFormCriar({...formCriar,senha:e.target.value})}/>{erroAcao && <p className="rounded-lg bg-red-50 p-3 text-sm text-red-700 dark:bg-red-900/20 dark:text-red-300">{erroAcao}</p>}<div className="flex justify-end gap-3"><Button size="sm" variant="outline" onClick={criarModal.closeModal}>Cancelar</Button><Button size="sm" disabled={criar.loading}>{criar.loading?"Criando...":"Criar"}</Button></div></form></Modal>
