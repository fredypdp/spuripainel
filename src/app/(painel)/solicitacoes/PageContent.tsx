  ...
  return (
    <div>
      ...
      <div className="space-y-6">
        ...
        {(isEstudante || isAcademia) && !editSelecionada && (
          <section className="rounded-2xl border border-gray-200 bg-white p-3 dark:border-gray-800 dark:bg-white/[0.03]">
            <div className="hidden flex-wrap gap-2 sm:flex">
              {abasSolicitacoes.map((item) => (
                <button key={item.value} type="button" onClick={() => { setAba(item.value); if (item.value !== "edicao") setTipo(item.value); }} className={`rounded-lg px-4 py-2 text-sm font-medium transition ${aba === item.value ? "bg-brand-500 text-white" : "border border-gray-200 text-gray-700 hover:bg-gray-50 dark:border-gray-700 dark:text-gray-300 dark:hover:bg-gray-800"}`}>
                  {item.label}
                </button>
              ))}
            </div>
            <div className="sm:hidden">
              <SearchableSelect value={aba} options={abasSolicitacoes} onChange={(value) => { const nextAba = value as AbaSolicitacao; setAba(nextAba); if (nextAba !== "edicao") setTipo(nextAba); }} isSearchable={false} />
            </div>
          </section>
        )}
