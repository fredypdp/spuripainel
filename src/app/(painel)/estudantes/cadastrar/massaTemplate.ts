...
function montarLinhasInstrucoes(contexto: ContextoModelo): (string | undefined)[][] {
  ...
  linhas.push(['Ano / Turma', contexto.anoAcademicoLabel]);
  ...
  linhas.push([
    `3. Não é preciso indicar curso ou ano acadêmico: este modelo já está definido para o contexto acima. O limite é de ${LIMITE_ESTUDANTES_POR_LOTE} estudantes por envio.`,
  ]);
  linhas.push(['4. A Data de Nascimento deve ser escrita como texto no formato DD/MM/AAAA. Exemplo: 15/05/2010.']);
  linhas.push(['5. O Género deve ser escrito exatamente como "masculino" ou "feminino".']);
  linhas.push(['6. Os telefones devem ter exatamente 9 dígitos, sem +244, espaços ou traços. Exemplo: 923456789.']);
  linhas.push(['7. O Bilhete de Identidade segue o formato 123456789LA041 (9 números, 2 letras, 3 números).']);
  linhas.push(['8. Os documentos (BI, cédula, certificados) são anexados depois, individualmente, na ficha de cada estudante.']);

  if (contexto.anoAcademico === '1_ano_fundamental') {
    linhas.push([
      '9. Para o 1º Ano Fundamental, deixe o Bilhete de Identidade do Estudante em branco — este ano usa Cédula, anexada depois.',
    ]);
  }
...
export function gerarModeloExcel(contexto: ContextoModelo): void {
  ...
  for (let r = 1; r <= LIMITE_ESTUDANTES_POR_LOTE; r++) {
  ...
  wsDados['!ref'] = XLSX.utils.encode_range({
    s: { r: 0, c: 0 },
    e: { r: LIMITE_ESTUDANTES_POR_LOTE, c: COLUNAS_MODELO_MASSA.length - 1 },
  });
