...
function DadosPessoaisSection({ user, onUpdated }: { user: MeuPerfilResponse; onUpdated: () => Promise<unknown> | void }) {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    setSaving(true);
    try {
      if (isAcademia) await academiaService.atualizarDados({ nome: nome || undefined, endereco: form.endereco.trim() || undefined, website: form.website.trim() || undefined });
...
function ContatoSection({ user, onUpdated }: { user: MeuPerfilResponse; onUpdated: () => Promise<unknown> | void }) {
  const initial = useMemo(() => getContatoInitial(user), [user]);
  const [email, setEmail] = useState(initial.email); const [telefone, setTelefone] = useState(initial.telefone);
  const [saving, setSaving] = useState<"email" | "telefone" | null>(null); const [success, setSuccess] = useState(""); const [error, setError] = useState("");
  useEffect(() => { setEmail(initial.email); setTelefone(initial.telefone); }, [initial]);
  async function salvarEmail(event: React.FormEvent) { event.preventDefault(); setSuccess(""); setError(""); const valor = email.trim(); if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(valor)) return setError("Informe um email válido."); if (valor === initial.email.trim()) return setError("Altere o email antes de guardar."); setSaving("email"); try { await perfilService.atualizarEmail({ email: valor }); await onUpdated(); setSuccess("Email atualizado com sucesso."); } catch (err) { setError(formatApiError(err, "Não foi possível atualizar o email.")); } finally { setSaving(null); } }
  async function salvarTelefone(event: React.FormEvent) { event.preventDefault(); setSuccess(""); setError(""); const valor = telefone.trim(); if (valor.length !== 9) return setError("Telefone deve ter exatamente 9 dígitos locais."); if (valor === initial.telefone.trim()) return setError("Altere o telefone antes de guardar."); setSaving("telefone"); try { await perfilService.atualizarTelefone({ telefone: valor }); await onUpdated(); setSuccess("Telefone atualizado com sucesso."); } catch (err) { setError(formatApiError(err, "Não foi possível atualizar o telefone.")); } finally { setSaving(null); } }
  return <section className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900"><CardHeader icon="mdi:card-account-mail-outline" title="Email e telefone" description="Atualize cada forma de contato separadamente." /><div className="space-y-5"><form onSubmit={salvarEmail} className="rounded-xl border border-gray-100 p-4 dark:border-gray-800"><div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-end"><Field id="personalizar-email" label="Email" type="email" value={email} disabled={saving !== null} onChange={setEmail} /><button type="submit" disabled={saving !== null} className="rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50">{saving === "email" ? "A guardar..." : "Salvar email"}</button></div></form><form onSubmit={salvarTelefone} className="rounded-xl border border-gray-100 p-4 dark:border-gray-800"><div className="grid gap-3 md:grid-cols-[1fr_auto] md:items-end"><Field id="personalizar-telefone" label="Telefone" type="tel" value={telefone} disabled={saving !== null} onChange={(value) => setTelefone(onlyDigits(value))} /><button type="submit" disabled={saving !== null} className="rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50">{saving === "telefone" ? "A guardar..." : "Salvar telefone"}</button></div></form></div>{error && <div className="mt-4"><Alert title="Erro ao atualizar contato" message={error} variant="error" /></div>}{success && <div className="mt-4"><Alert title="Contato atualizado" message={success} variant="success" /></div>}</section>;
}
