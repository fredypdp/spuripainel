...
export default function AvaliacaoFinalRulesSection() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    const payload = { type: type || "normal", nome, descricao: descricao || undefined, tipo_ensino: tipoSelecionado, anos_academicos: [...anos], nota_minima_aprovacao: Number(notaMinima), categorias_envolvidas: categoriasEnvolvidas, formula: formula.trim(), aplica_se_reprovado_em_type: dependencia || null };
  ...
  const canSubmit = !!nome.trim() && !!notaMinima && Number(notaMinima) > 0 && anos.size > 0 && categoriasEnvolvidas.length > 0 && !!formula && !precisaValor;

  return <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
    <div className="mb-5"><h2 className="flex items-center gap-2 text-lg font-bold text-gray-800 dark:text-white"><span className="flex h-7 w-7 items-center justify-center rounded-lg bg-brand-50 dark:bg-brand-500/10"><Icon icon="mdi:function-variant" width="16px" className="text-brand-500" /></span>Regras de avaliação final</h2><p className="mt-1 max-w-3xl text-sm text-gray-500 dark:text-gray-400">Monte a regra passo a passo. O texto da fórmula é apenas uma pré-visualização; a criação é feita pelos botões, para evitar erros de digitação.</p></div>
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <form onSubmit={submit} className="space-y-4">
        <div className="rounded-xl border border-gray-200 p-4 dark:border-gray-800"><p className="mb-3 text-sm font-semibold text-gray-800 dark:text-white">1. Identificação da regra</p><div className="grid gap-3 sm:grid-cols-2">{isMista ? <label className="text-xs font-medium text-gray-600 dark:text-gray-300">Tipo de academia<select value={tipoSelecionado} onChange={(e) => handleTipoEnsinoChange(e.target.value as TipoEnsino)} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white"><option value="fundamental">Ensino fundamental</option><option value="medio">Ensino médio</option></select></label> : <div className="rounded-lg bg-gray-50 px-3 py-2.5 text-sm text-gray-700 dark:bg-gray-800 dark:text-gray-200"><span className="block text-xs font-medium text-gray-500">Tipo de academia</span>{labelTipo(tipoSelecionado)}</div>}<label className="text-xs font-medium text-gray-600 dark:text-gray-300">Etapa da regra<input value={type} onChange={(e) => setType(e.target.value)} placeholder="Ex.: normal, recurso, especial" className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white" /></label><label className="text-xs font-medium text-gray-600 dark:text-gray-300">Nome da regra<input value={nome} onChange={(e) => setNome(e.target.value)} placeholder="Ex.: Média anual" className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white" /></label><label className="text-xs font-medium text-gray-600 dark:text-gray-300">Nota mínima de aprovação<input type="number" min={1} value={notaMinima} onChange={(e) => setNotaMinima(e.target.value)} placeholder="Ex.: 10" className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white" /></label></div><label className="mt-3 block text-xs font-medium text-gray-600 dark:text-gray-300">Descrição<textarea value={descricao} onChange={(e) => setDescricao(e.target.value)} placeholder="Explique quando esta regra deve ser usada" rows={2} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white" /></label><label className="mt-3 block text-xs font-medium text-gray-600 dark:text-gray-300">Dependência<select value={dependencia} onChange={(e) => setDependencia(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white"><option value="">Regra raiz (primeira avaliação)</option>{regras.filter(r => r.tipo_ensino === tipoSelecionado).map(r => <option key={r.id} value={r.type}>Só aplicar se reprovar em {r.type}</option>)}</select></label></div>
