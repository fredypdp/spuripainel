  ...
  return <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="rounded-xl border border-gray-200 p-4 dark:border-gray-800"><p className="text-sm font-semibold text-gray-800 dark:text-white">3. Fórmula guiada</p><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">Monte a conta passo a passo, adicionando uma nota ou número e depois escolhendo a operação desejada. O sistema extrai automaticamente as categorias usadas na fórmula e calcula a nota final assim que todas as notas necessárias forem lançadas.</p><div className="mt-3 rounded-lg bg-gray-50 p-3 font-mono text-xs text-gray-700 dark:bg-gray-800 dark:text-gray-200">{formula || "A fórmula aparecerá aqui conforme você adicionar notas, números e operações."}</div>{precisaValor ? <div className="mt-3 space-y-3"><div className="grid items-end gap-2 sm:grid-cols-[1fr_1fr_auto]"><label className="block text-xs font-medium text-gray-600 dark:text-gray-300">Categoria da nota<select value={draftCategoria} onChange={(e) => setDraftCategoria(e.target.value)} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white"><option value="">Categoria da nota</option>{categoriasDisponiveis.map(c => <option key={c} value={c}>{c}</option>)}</select></label><label className="block text-xs font-medium text-gray-600 dark:text-gray-300">Período<select value={draftPeriodo} onChange={(e) => setDraftPeriodo(e.target.value)} disabled={periodosFormula.length === 0} className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm disabled:opacity-50 dark:border-gray-700 dark:bg-gray-800 dark:text-white"><option value="">Período</option>{periodosFormula.map(p => <option key={p} value={p}>{labelPeriodo(p)}</option>)}</select></label><button type="button" onClick={addRef} disabled={!draftCategoria || !draftPeriodo} className="inline-flex h-10 items-center justify-center rounded-lg bg-brand-500 px-3 text-sm font-medium text-white disabled:opacity-50">Adicionar nota</button></div><div className="grid items-end gap-2 sm:grid-cols-[1fr_auto]"><label className="block text-xs font-medium text-gray-600 dark:text-gray-300">Número constante<input type="number" min={0} step="0.01" value={draftConstante} onChange={(e) => setDraftConstante(e.target.value)} placeholder="Número constante" className="mt-1 w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-800 dark:text-white" /></label><button type="button" onClick={addConstante} disabled={!draftConstante} className="inline-flex h-10 items-center justify-center rounded-lg border border-gray-300 px-3 text-sm font-medium text-gray-700 disabled:opacity-50 dark:border-gray-700 dark:text-gray-200">Adicionar número</button></div></div> : <div className="mt-3 flex flex-wrap gap-2">{["+", "-", "*", "/"].map(op => <button type="button" key={op} onClick={() => setFormulaItems((prev) => [...prev, { kind: "op", op: op as "+" | "-" | "*" | "/" }])} className="h-10 w-10 rounded-lg bg-brand-50 text-lg font-bold text-brand-600 dark:bg-brand-900/20 dark:text-brand-300">{op}</button>)}</div>}<div className="mt-3 flex gap-2"><button type="button" onClick={() => setFormulaItems((prev) => prev.slice(0, -1))} className="text-xs font-medium text-gray-500 hover:text-gray-700">Desfazer último item</button><button type="button" onClick={() => setFormulaItems([])} className="text-xs font-medium text-red-500 hover:text-red-600">Limpar fórmula</button></div></div>
