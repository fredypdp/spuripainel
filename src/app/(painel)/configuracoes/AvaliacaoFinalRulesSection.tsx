...
export default function AvaliacaoFinalRulesSection() {
  ...
  const [formula, setFormula] = useState("([nota_escola,1_trimestre]+[nota_professor,1_trimestre]+[nota_escola,2_trimestre]+[nota_professor,2_trimestre]+[nota_escola,3_trimestre]+[nota_professor,3_trimestre])/3");
  ...
  function handleTipoEnsinoChange(nextTipo: TipoEnsino) {
    ...
    const nextCategorias = nextTipo === "superior" ? ["nota_pp1", "nota_pp2"] : ["nota_escola", "nota_professor"];
    setCategorias(new Set(nextCategorias));
    setFormula(buildFormulaText(nextCategorias, [], nextTipo));
    setAnos(new Set());
  }
  ...
  function buildFormulaText(cats = [...categorias], selectedAnos = [...anos], ensino = tipoEnsino): string {
    const periodos = ensino === "superior" ? selectedAnos : ESCOLAR_PERIODOS;
    const refs = periodos.flatMap((periodo) => cats.map((categoria) => `[${categoria},${periodo}]`));
    return refs.length > 1 ? `(${refs.join("+")})/${Math.max(1, periodos.length)}` : refs[0] ?? "";
  }

  function sugerirFormula() {
    setFormula(buildFormulaText());
  }

  async function submit(e: React.FormEvent) {
    e.preventDefault();
    setSucesso(null);
    const payload = { type: type || "normal", nome, descricao: descricao || undefined, tipo_ensino: tipoEnsino, anos_academicos: [...anos], nota_minima_aprovacao: notaMinima, categorias_envolvidas: [...categorias], formula: formula.trim(), aplica_se_reprovado_em_type: dependencia || null };
  ...
  return <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
    ...
    <div className="grid gap-6 lg:grid-cols-2">
      ...
      <form onSubmit={submit} className="space-y-3">
        ...
        <div className="rounded-lg border border-gray-200 p-3 dark:border-gray-800"><div className="mb-2 flex flex-wrap items-center justify-between gap-2"><p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Fórmula textual *</p><button type="button" onClick={sugerirFormula} className="text-xs font-medium text-brand-500 hover:text-brand-600">Sugerir média</button></div><textarea value={formula} onChange={(e) => setFormula(e.target.value)} rows={4} placeholder="([nota_escola,1_trimestre]+[nota_escola,2_trimestre]+[nota_escola,3_trimestre])/3" className="w-full rounded-lg border border-gray-300 bg-white px-3 py-2.5 font-mono text-xs dark:border-gray-700 dark:bg-gray-800 dark:text-white" /><p className="mt-2 text-xs text-gray-500 dark:text-gray-400">Use a sintaxe formula_textual_v1: referências [categoria,periodo], operadores + - * /, parênteses e constantes positivas. O backend valida categorias, períodos, divisão por zero e caracteres inválidos.</p></div>
        ...
        <button type="submit" disabled={criando || !nome || anos.size === 0 || categorias.size === 0 || !formula.trim()} className="inline-flex w-full items-center justify-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white hover:bg-brand-600 disabled:opacity-50">{criando ? "A criar..." : "Criar regra"}</button>
