...
function certificadoFieldForAno(ano?: string | null): FileKey | null {
  if (!ano) return null;
  if (["7_ano_fundamental", "8_ano_fundamental", "9_ano_fundamental"].includes(ano)) return "certificado_6_ano_fundamental";
  if (isMedio(ano)) return "certificado_9_ano_fundamental";
  if (isSuperior(ano)) return "certificado_ensino_medio";
  return null;
}
...
export default function MatriculaPublicPage() {
  ...
  const documentos = useMemo<DocumentoOpcao[]>(() => {
    ...
    const certificado = certificadoFieldForAno(anoSelecionado);
    if (certificado) {
      docs.push({ key: "declaracao", label: "Declaração escolar (alternativa ao certificado)", obrigatorio: !files[certificado] });
    }
    if (certificado === "certificado_6_ano_fundamental") {
      docs.push({ key: certificado, label: "Certificado do 6.º ano fundamental", obrigatorio: !files.declaracao });
    }
    if (certificado === "certificado_9_ano_fundamental") {
      docs.push({ key: certificado, label: "Certificado do 9.º ano fundamental", obrigatorio: !files.declaracao });
    }
    if (certificado === "certificado_ensino_medio") {
      docs.push({ key: certificado, label: "Certificado do ensino médio", obrigatorio: !files.declaracao });
    }
    return docs;
