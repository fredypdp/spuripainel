...
function ValorNotaComCorrecao({ nota }: { nota?: Nota }) {
  if (!nota) return null;
  return <span title={tituloCorrecaoNota(nota)}>{notaText(nota.nota)}<NotaCorrigidaBadge nota={nota} />{nota.motivo_correcao && <span className="sr-only"> Motivo da correção: {nota.motivo_correcao}</span>}</span>;
}
            ...
            .map(({ codigo, nome, ne }) => (
              <tr key={codigo} className="bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/80 transition-colors">
                ...
                {categoriasOrdem.map((cat) => {
                  ...
                  return <td key={cat} className={`px-4 py-3 text-right font-bold ${notaCat ? corNota(notaCat.nota) : "text-gray-300 dark:text-gray-600"}`}>{<ValorNotaComCorrecao nota={notaCat} />}</td>;
                })}
              </tr>
            ))}
        </tbody>
...
export default function NotasAdmin() {
  ...
  useEffect(() => {
    ...
    const idsConfiguradas = materias
      .filter((m: any) =>
        ...
        ((acadLayer.mode !== "sup") || !m.periodo || m.periodo === l.periodo)
      )
      .map((m: any) => m.id);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [acadLayer, materias, materiasCache, notasPorEstudante]);
  ...
  if (navLayer.type === "academias") {
    ...
    return (
      <div className="space-y-6">
        {alert && <Alert variant={alert.variant} title={alert.variant === "success" ? "Sucesso" : "Erro"} message={alert.message} />}
        <button type="button" onClick={() => setNavLayer({ type: "provincias" })}
          className="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3 py-2 text-sm font-medium text-gray-700 shadow-sm transition hover:border-brand-300 hover:bg-brand-50 hover:text-brand-700 focus:outline-none focus:ring-2 focus:ring-brand-500/20 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:border-brand-700 dark:hover:bg-brand-900/20 dark:hover:text-brand-300"
        >
          <Icon icon="mdi:arrow-left" width={18} /> Voltar
        </button>
