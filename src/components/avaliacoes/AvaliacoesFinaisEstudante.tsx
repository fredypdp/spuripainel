      ...
      {avs.length > 0 && (
        <div className="text-center">
          ...
          <p className="text-xl font-bold text-brand-600 dark:text-brand-400">
            {Math.round((aprovadas / avs.length) * 100)}%
          </p>
        </div>
      )}
    </div>
...
function TabelaCiclo({ avs }: { avs: AvaliacaoFinal[] }) {
  if (avs.length === 0) {
    return (
    ...
    <div className="overflow-x-auto rounded-xl border border-gray-200 dark:border-gray-700">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-gray-100 dark:divide-gray-700/50">
          {sortAvs(avs).map((a, idx) => (
            <tr
              key={a.id}
              className={`transition-colors ${
                a.aprovado
                  ? "bg-white dark:bg-gray-800 hover:bg-emerald-50/30 dark:hover:bg-emerald-900/10"
                  : "bg-white dark:bg-gray-800 hover:bg-red-50/30 dark:hover:bg-red-900/10"
              }`}
            >
...
export default function AvaliacoesFinaisEstudante() {
  ...
  const { data, execute: carregar, loading } = useApi(
    consultasService.listarAvaliacoes
  );
  ...
  // Determine which ciclos the student has data for
  const ciclosPresentes = useMemo((): TipoEnsino[] => {
    ...
    // Also include ciclos where student is currently enrolled
  ...
  const avsPorCiclo = (ciclo: TipoEnsino) =>
    todasAvaliacoes.filter(
      a => getTipoEnsino(a.ano_academico_atual) === ciclo
    );

  const cicloInfo: Record<
    TipoEnsino,
      ...
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Historial das avaliações anuais por ciclo de ensino
          </p>
        </div>
        ...
        {ciclosPresentes.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">
              Ainda não tens avaliações finais registadas.
            </p>
            <p className="text-xs text-gray-400 mt-1">
              As avaliações são registadas pela tua instituição de ensino.
            </p>
          </div>
        ) : (
