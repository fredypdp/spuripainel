...
export default function AcademiaCadastroForm({ onSubmit, submitting, apiError, showSenhaField = false, submitLabel = "Cadastrar", submittingLabel = "Cadastrando...", infoNote, secondaryAction }: AcademiaCadastroFormProps) {
  ...
  const validarFormulario = (): boolean => {
    ...
    if (showSenhaField) {
      ...
      else if (senha.trim().length < 8) erros.push('A senha deve ter no mínimo 8 caracteres');
  ...
  return (
    <form onSubmit={handleSubmit}>
      <div className="grid grid-cols-1 gap-x-6 gap-y-5 sm:grid-cols-2">
        ...
        {showSenhaField && <div className="col-span-2 sm:col-span-1"><Label>Senha *</Label><Input type="password" placeholder="Mínimo 8 caracteres" value={senha} onChange={(e) => setSenha(e.target.value)} disabled={submitting} /><p className="mt-1 text-xs text-gray-500 dark:text-gray-400">Esta senha será usada para acessar a conta após a ativação.</p></div>}
